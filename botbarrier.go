...
const (
	...
	maxComplexity     = 32
	seedSizeBytes     = 16
	solutionSizeBytes = 16
	macSizeBytes      = sha512.Size
)
...
func (bb *BotBarrier) checkSolution(r *http.Request, complexity int, logger *zap.Logger) bool {
	...
	if len(seedCookie.Value) != seedSizeBytes*2 {
		logger.Warn("Invalid seed length", zap.Int("seed_length", len(seedCookie.Value)))
		return false
	}
	seedBytes, err := hex.DecodeString(seedCookie.Value)
	if err != nil {
		logger.Warn("Invalid seed format", zap.Error(err))
		return false
	}
	...
	if len(macCookie.Value) != macSizeBytes*2 {
		logger.Warn("Invalid MAC length", zap.Int("mac_length", len(macCookie.Value)))
		return false
	}
	macBytes, err := hex.DecodeString(macCookie.Value)
	if err != nil || !hmac.Equal(macBytes, bb.createMAC(seedBytes)) {
		logger.Warn("Invalid MAC", zap.Error(err))
		return false
	}

	// Decode the solution (nonce) from the cookie
	if len(solutionCookie.Value) != solutionSizeBytes*2 {
		logger.Warn("Invalid solution length", zap.Int("solution_length", len(solutionCookie.Value)))
		return false
	}
	solutionBytes, err := hex.DecodeString(solutionCookie.Value)
	if err != nil {
		logger.Warn("Invalid solution", zap.Error(err))
		return false
	}
