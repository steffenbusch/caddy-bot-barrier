...
func (bb *BotBarrier) Provision(ctx caddy.Context) error {
	...
	bb.logger.Info("BotBarrier module provisioned successfully",
		zap.Int("complexity", bb.Complexity),
...
func (bb *BotBarrier) Validate() error {
	...
	// Validate that complexity is within a realistic range
	if bb.Complexity <= 0 || bb.Complexity > 64 {
		return fmt.Errorf("complexity must be between 1 and 64, got %d", bb.Complexity)
	}
...
func (bb BotBarrier) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	bb.logger.Debug("ServeHTTP called", zap.String("method", r.Method), zap.String("url", r.URL.String()))

	if bb.checkSolution(r, clientIP) {
		bb.logger.Info("Client passed the computational challenge", zap.String("client_ip", clientIP))
		return next.ServeHTTP(w, r)
	}

	bb.logger.Info("Client did not pass the computational challenge, serving challenge page", zap.String("client_ip", clientIP))

	seed, err := bb.newSeed()
	if err != nil {
		bb.logger.Error("Failed to generate seed", zap.Error(err))
		http.Error(w, "Seed generation failed", http.StatusInternalServerError)
		return nil
	}
	seedHex := hex.EncodeToString(seed)
	mac := bb.createMAC(seed)
	bb.logger.Debug("Generated seed and MAC", zap.String("seed", seedHex), zap.String("mac", hex.EncodeToString(mac)))
	...
	bb.logger.Info("Challenge page served successfully", zap.String("client_ip", clientIP))
	return nil
}
...
func (bb BotBarrier) checkSolution(r *http.Request, clientIP string) bool {
	bb.logger.Debug("Checking computational challenge solution", zap.String("client_ip", clientIP))
	...
	if errSeed != nil || errSolution != nil || errMac != nil {
		bb.logger.Warn("Missing required cookies", zap.Error(errSeed), zap.Error(errSolution), zap.Error(errMac))
		return false
	}

	// Decode the seed from the cookie
	seedBytes, err := hex.DecodeString(seedCookie.Value)
	if err != nil || !bb.isSeedValid(seedBytes) {
		bb.logger.Warn("Invalid or expired seed", zap.String("seed", seedCookie.Value), zap.Error(err))
		return false
	}
	...
	if err != nil || !hmac.Equal(macBytes, bb.createMAC(seedBytes)) {
		bb.logger.Warn("Invalid MAC", zap.String("mac", macCookie.Value), zap.Error(err))
		return false
	}
	...
	if err != nil {
		bb.logger.Warn("Invalid solution", zap.String("solution", solutionCookie.Value), zap.Error(err))
		return false
	}
	...
	if leadingZeroBits >= bb.Complexity {
		bb.logger.Info("Computational challenge solution valid",
			zap.Int("leading_zero_bits", leadingZeroBits),
			zap.Int("required_complexity", bb.Complexity),
		)
		return true
	}

	bb.logger.Warn("Computational challenge solution invalid",
		zap.Int("leading_zero_bits", leadingZeroBits),
		zap.Int("required_complexity", bb.Complexity),
	)
	return false
...
func (bb BotBarrier) isSeedValid(seed []byte) bool {
	if len(seed) < 8 {
	...
	bb.logger.Warn("Seed expired", zap.Duration("age", age), zap.Duration("valid_for", bb.ValidFor))
	return false
}
